...
interface Props {
  searchParams: Promise<{ status?: string; reason?: string; id?: string }>;
}

const ERROR_MESSAGES: Record<string, { title: string; body: string }> = {
  missing: {
    title: 'Ugyldigt link',
    body: 'Bekræftelseslinket mangler en token. Brug linket fra din email eller tilmeld dig igen.',
  },
  invalid: {
    title: 'Link allerede brugt eller ugyldigt',
    body: 'Dette bekræftelseslink er allerede brugt eller ugyldigt. Hvis din email er bekræftet, kan du logge ind. Ellers kan du tilmelde dig igen.',
  },
  expired: {
    title: 'Linket er udløbet',
    body: 'Bekræftelseslinket er udløbet — det er gyldigt i 24 timer. Tilmeld dig igen på forsiden for at modtage et nyt link.',
  },
};

export default async function ConfirmPage({ searchParams }: Props) {
  const { status, reason, id } = await searchParams;

  const isSuccess = status === 'success';
  const company = isSuccess && id ? await getCompany(id) : null;

  const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || 'https://aisignal.dk';

  if (isSuccess && !company) {
    // Verified company not found — redirect home
    return (
            ...
            <>
              ...
              >
                ...
                <p style={{ margin: 0, fontSize: 14, color: '#d4d4d8', lineHeight: 1.6 }}>
                  Din første AI-analyse er igangsat og klar inden for de næste par minutter. Du
                  modtager derefter ugentlige rapporter — næste aflevering er{' '}
